      ...
      <div class="breadcrumb-main">
        <div class="container">
          <div class="breadcrumb-container-inner" <?php echo(count($styles_inner) > 0 ? 'style="' . implode(';', $styles_inner) . '"' : ''); ?>>
           <?php 
            if ($title && $show_page_title) { 
                if (is_single()) {
                    // If it's a single post page, display a different heading
                    echo '<h2 class="heading-title">';
                    the_title();
                    echo '</h2>';
                } else {
                    // If it's not a single post page, display the regular page title
